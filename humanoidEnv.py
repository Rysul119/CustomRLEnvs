...
class humanoidBiped(gym.Env):
    ...
    def __init__(self, action_dim, obs_dim):
        ...
        self.power = 0.40 # hard coding the power value
        self.targetX = 1e3
        self.targetY = 0
    ...
    def robot(self, bullet_client):
        ...
        for j in range(self._p.getNumJoints(self.botId)):
            ...
            if (jointName[:8] != 'jointfix'):
                ...
                jointDict[jointName].power_coeff = 30
    ...
    def get_rewards(self, z, action):
        if(z > 0.5 and z < 1.3 and abs(self.baseP) < 1.0):
            self._alive = 1.0
        else:
            self._alive = - 10.0
    ...
    def reset(self):
        ...
        _, self.botId = self._p.loadMJCF("bipedModel.xml")
        #self.planeId = self._p.loadURDF('plane.urdf', [0, 0, 0])
        #self.rewards = 0
        #self.done = 0
